        ...
        <section className="min-h-[80vh] flex flex-col lg:flex-row items-center gap-12 mb-32">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4 pt-6">
              <Button asChild size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-medium glow-primary group">
                <Link to="/experience">
                  View Experience
                  <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                </Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-secondary text-secondary hover:bg-secondary/10 font-medium">
                <Link to="/contact">
                  Contact Me
          ...
          >
            <div className="relative w-80 h-80">
              {/* Central Avatar */}
              <div className="absolute inset-0 rounded-full bg-gradient-to-br from-primary via-secondary to-primary glow-primary flex items-center justify-center">
                <div className="w-[95%] h-[95%] rounded-full bg-background flex items-center justify-center">
                  <span className="text-8xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-br from-primary to-secondary">
                    SN
                  </span>
                </div>
              </div>
              ...
              {floatingChips.map((chip, index) => (
                ...
                >
                  <motion.div
                    ...
                    className="glass px-4 py-2 rounded-full border border-primary/30 text-sm font-medium whitespace-nowrap glow-primary"
                  >
                    {chip.text}
                  </motion.div>
                </motion.div>
            ...
            {quickLinks.map((link, index) => (
              <Link key={link.path} to={link.path}>
                <GlassCard delay={index * 0.1} className="h-full">
                  ...
                  <h3 className="text-xl font-display font-semibold mb-2">{link.title}</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <GlassCard>
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <h3 className="text-2xl font-display font-bold">Mirage</h3>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["React", "D3.js", "FastAPI", "LangChain", "ChromaDB"].map((tech) => (
                      <TechBadge key={tech} name={tech} variant="secondary" />
                    ))}
                  </div>
                  <div className="pt-4 flex items-center text-primary font-medium group">
                    View Case Study
            ...
            <Link to="/projects">
              <GlassCard>
                <div className="space-y-4">
                  <div className="flex items-start justify-between">
                    <h3 className="text-2xl font-display font-bold">Dark Web Traffic Detection</h3>
                    <span className="text-xs px-3 py-1 rounded-full bg-secondary/20 text-secondary border border-secondary/30">
                      2024
                    </span>
                  </div>
